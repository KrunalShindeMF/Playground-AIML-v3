...
# context = vectordb.similarity_search("give me some suggestion for savings", k=5)
...
#     return token, hashed_token, expiration_time

# def send_password_reset_email(to_email, reset_link):
#     message = Mail(
#         from_email='pratik.mobilefirst@gmail.com',
#         to_emails=to_email,
#         subject='Password Reset Request',
#         html_content=f"<p>Click the following link to reset your password: <a href='{reset_link}'>Reset Password</a></p>"
#     )
#     try:
...
def generate_session_id(document):
    ...
    document.seek(0)  # Reset the file pointer to the beginning of the file
    hash_bytes = hasher.digest()
    # Convert hash to base64 and then filter alphabetic characters
    hash_base64 = base64.b64encode(hash_bytes).decode('utf-8')
    session_id = re.sub(r'[^a-zA-Z]', '', hash_base64)
    return session_id[:32]  # Trim or pad to desired length
...
#                     {
#                         "type": "text",
#                         "text": "give all the data from all the images in text form and remember don't break sequence of images need to provide text with respect to image sequence."
#                     },
#                     {
#                         "type": "image_url",
#                         "image_url": {
...
def process_image_1(image_path):
    ...
    response = client.chat.completions.create(
        ...
        #     {
        #         "role": "user",
        #         "content": "Extract all text from the provided image without adding any extra lines or suggestions."
        #     },
        #     {
        #         "role": "user",
...
def convert_pdf_to_images(pdf_path, output_folder, image_format='jpg', quality=85):
    ...
    for page_num in range(len(pdf_document)):
        ...
        if image_format.lower() == 'jpg':
            pix.save(image_path)  # Save as JPEG with compression
...
def extract_text_from_msg(msg_file_path):
    try:
        ...
        # Optionally, extract attachments if needed
        attachments = msg.attachments

        # Combine all text content
        full_text = f"From: {msg_sender}\nDate: {msg_date}\nSubject: {msg_subject}\n\n{msg_body}"

        # Optionally, process attachments
...
def extract_tables_from_docx(file_path):
    ...
    for table in doc.tables:
        ...
        # Append the parsed table to the list
...
checkbox_output = """
    ...
    NOTE : Do not break sequence. You need to give response in proper sequence as it is in given data.
"""
